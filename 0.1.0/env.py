...
class MetaworldEnv(gym.Env):
    ...
    def __init__(self,
        ...
        cam_id = env.sim.model.camera_name2id('corner2')
        assert cam_id == 2      # human knowledge
        env.sim.model.cam_pos0[cam_id] = [0.6, 0.295, 0.8]
        env.sim.model.cam_pos[cam_id] = [0.6, 0.295, 0.8]

        # setup camera properties
        camera_ids = {name: env.sim.model.camera_name2id(name) for name in camera_names}

        camera_o3d_pinhole = {
            name: o3d.camera.PinholeCameraIntrinsic(
                image_size, image_size,
                image_size / (2 * np.tan(np.radians(env.sim.model.cam_fovy[cam_id]) / 2)),
                image_size / (2 * np.tan(np.radians(env.sim.model.cam_fovy[cam_id]) / 2)),
        ...
        for cam_name, cam_id in camera_ids.items():
            camera_poses[cam_name][:3, 3] = env.sim.model.cam_pos0[cam_id]
            camera_poses[cam_name][:3, :3] = np.matmul(
                env.sim.model.cam_mat0[cam_id].reshape(3, 3), 
